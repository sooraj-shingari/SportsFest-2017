            ...
            if(count($q_team)==0){
               ...
               // return Redirect('/');
            	...
            	if(count($qw)>0){
            	 ...
            	 $a="https://secure.paytm.in/oltp/HANDLER_INTERNAL/TXNSTATUS?JsonData={'MID':'MID','ORDERID':'".$qw->Order_Id."'}";
...
public function check(){
	...
	//echo json_encode($d->User_Id);
		 $a="https://secure.paytm.in/oltp/HANDLER_INTERNAL/TXNSTATUS?JsonData={'MID':'MID','ORDERID':'".$d->Order_Id."'}";
	            $contents = file_get_contents($a); 
	            $contents=json_decode($contents,true);
	...
	//foreach($q_order as $q){
            $a="https://secure.paytm.in/oltp/HANDLER_INTERNAL/TXNSTATUS?JsonData={'MID':'MID','ORDERID':'ORDER972'}";
            $contents = file_get_contents($a); 
            $contents=json_decode($contents,true);
